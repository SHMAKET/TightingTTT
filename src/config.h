...
#define MAX_BRIGHTNESS_LED 255     // Яркость (0-255)
...
const unsigned long syncIntervalMs = 3600000; // 1 час = 3600*1000 мс интервал синхронизации времени с NTP сервером
//                   СС  ММ  ЧЧ
const tm glowTime  {  3,  0,  0 };      // время свечения после срабатывания датчика + задержка самого датчика (2-3c)
const tm StartTime {  0, 30,  7 }; //{ 0, 30, 22 };       // врямя начала опроса датчика
const tm EndTime   {  0, 30,  6 };      // время окночания опроса датчика

const int TIME_ON  = 500;               // время плавного включения ленты (мс)
const int TIME_OFF = 1000;              // время плавного выключения ленты (мс)